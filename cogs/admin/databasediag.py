...
class DatabaseDiag(commands.Cog):
    ...
    async def check_db_integrity(self, ctx: commands.Context):
        try:
            ...
            rows = await postgres.fetch("SELECT * FROM user_data WHERE discordID IS NULL OR discordName IS NULL OR preferredName IS NULL")
            if rows:
                issues.append(f"`user_data`: {len(rows)} row(s) have NULLs in required fields.")

            # Check userWarnings has valid discordIDs
            rows = await postgres.fetch("""
                SELECT uw.discordID FROM userWarnings uw
                LEFT JOIN user_data u ON u.discordID = uw.discordID
                WHERE u.discordID IS NULL
            """)
            if rows:
                issues.append(f"`userWarnings`: {len(rows)} row(s) reference nonexistent users in user_data.")

            # Check xiv_char has valid user_data links
            rows = await postgres.fetch("""
                SELECT c.discordID FROM xiv_char c
                LEFT JOIN user_data u ON u.discordID = c.discordID
                WHERE c.discordID IS NOT NULL AND u.discordID IS NULL
            """)
            if rows:
            ...
            for row in await postgres.fetch("SELECT * FROM consistentChannels"):
                ...
                # Always check if channel exists
                channel = ctx.guild.get_channel(channel_id)
                if channel is None:
                    bad_channels.append(channel_id)
                    continue  # Can't check message if channel is gone

                # Only check message if message_id is present
